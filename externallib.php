...
class local_start_external extends external_api {
     ...
     * @return array
     */
    public static function get_user($dados) {
        global $CFG, $DB, $USER;
		...
		// Get the user object.
        $usuario = $DB->get_record('user', array('id' => $dados['userid']), '*', MUST_EXIST);
		...
		$return = new stdClass();
		$return->id = (int) 1;
		$return->firstname = $usuario->firstname;
		$return->lastname = $usuario->lastname;
		$return->email = $usuario->email;
		$arrretorno[] = (array) $return;
        ...
        return new external_multiple_structure(
            new external_single_structure(
                array(
                    ...
                    'firstname' => new external_value(PARAM_TEXT, 'Firstname user'),
                    'lastname' => new external_value(PARAM_TEXT, 'Lastname user'),
     ...
     * @return external_function_parameters
     */
    public static function save_user_parameters() {
        return new external_function_parameters(
            array(
                'dados' => new external_single_structure(
                    array(
						...
						'lastname' => new external_value(PARAM_TEXT, 'Lastname user'),
						'email' => new external_value(PARAM_TEXT, 'Email user'),
						'password' => new external_value(PARAM_TEXT, 'Password user'),
						'password2' => new external_value(PARAM_TEXT, 'Confirm Password user')
                    )
                )
            )
        );
    ...
    /**
     * Returns save data user
     * @return string return
     */
    public static function save_user($dados) {
        global $CFG, $DB, $USER;
        ...
        //Parameter validation
        //REQUIRED
        $params = self::validate_parameters(self::save_user_parameters(),
                array('dados' => $dados));
        ...
        $usuario = $DB->update_record('user', array('id' => $dados['userid'],
													'firstname' => $dados['firstname'],
													'lastname' => $dados['lastname'],
													'email' => $dados['email']));

		$puser = $DB->get_record('user', array('id'=>$dados['userid']), '*', MUST_EXIST);
		
		if ($dados['password'] <> ""){
			if ($dados['password'] <> $dados['password2']){
			}else{
				if (update_internal_user_password($puser, $dados['password'])) {		
				} else {
				}
			}
		}
        ...
        $log = $DB->insert_record('log', array('userid' => $dados['userid'],
													...
													'module' => "Mobile",
													'action' => "Meus Dados - Alterar Senha",
													'url' => "",
													'info' => "Alterar Senha"));
     ...
     * @return external_description
     */
    public static function save_user_returns() {
     ...
     * @return external_function_parameters
     */
    public static function geoinfo_parameters() {
        return new external_function_parameters(
            array(
                'dados' => new external_single_structure(
                    array(
                        'userid' => new external_value(PARAM_INT, 'id of user'),
						'geoinfo' => new external_value(PARAM_TEXT, 'Device Info'),
                    )
                )
            )
        );
	...
	/**
     * Returns save device infor user
     * @return string return
     */
    public static function geoinfo($dados) {
        global $CFG, $DB, $USER;
        ...
        //Parameter validation
        //REQUIRED
        $params = self::validate_parameters(self::geoinfo_parameters(),
                array('dados' => $dados));
     ...
     * @return external_description
     */
    public static function geoinfo_returns() {
        return new external_value(PARAM_TEXT, 'Return data');
    }
     ...
     * @return external_function_parameters
     */
    public static function event_save_parameters() {
        return new external_function_parameters(
            array(
                'dados' => new external_single_structure(
                    array(
                        'relid' => new external_value(PARAM_INT, 'id of event'),
    ...
    /**
     * Returns event data save
     * @return string return
     */
    public static function event_save($dados) {
        global $CFG, $DB, $USER;
        ...
        //Parameter validation
        //REQUIRED
        $params = self::validate_parameters(self::event_save_parameters(),
                array('dados' => $dados));
        ...
        $usuario = $DB->update_record('event', array('id' => $dados['relid'],
													'name' => $dados['eventname']));
     ...
     * @return external_description
     */
    public static function event_save_returns() {
        return new external_value(PARAM_TEXT, 'Return message');
    }
     ...
     * @return external_function_parameters
     */
    public static function responder_forum_parameters() {
        return new external_function_parameters(
            array(
                'dados' => new external_single_structure(
                    array(
     ...
     * @return string return
     */
    public static function responder_forum($dados) {
        global $CFG, $DB, $USER;
        ...
        //Parameter validation
        //REQUIRED
        $params = self::validate_parameters(self::responder_forum_parameters(),
                array('dados' => $dados));
		...
		// Reply the post.
        $reply = $DB->insert_record('forum_posts', array('userid' => $dados['userid'],
													'parent' => $postid->id,
													'created' => $timestamp,
													'modified' => $timestamp,
													'discussion' => $dados['discussionId'],
													'subject' => "Re: ".$dados['subject'],
        ...
        $log = $DB->insert_record('log', array('userid' => $dados['userid'],
													...
													'ip' => $_SERVER['REMOTE_ADDR'],
													'module' => "Mobile",
													'action' => "Forum - Enviar Resposta",
													'url' => "",
													'info' => "Forum - Enviar Resposta: ".$dados['subject']));
	...
	/**
     * Describes the parameters for get_messages.
     *
     ...
     *
     * @param array $dados the forum ids
     * @return array the forum discussion details
     * @since Moodle 2.5
     */
    public static function get_messages($dados) {
        global $CFG, $DB, $USER;
		...
		// Array to store the messages to return.
        $arrmessages = array();
		
		// get the messages data
		if ($messages = $DB->get_records('message', array('useridto' => $dados['userid']))) {
		
			foreach ($messages as $message) {
				$nomefrom = $DB->get_record('user', array('id' => (int) $message->useridfrom), 'firstname,lastname', MUST_EXIST);
				$nometo = $DB->get_record('user', array('id' => (int) $message->useridto), 'firstname,lastname', MUST_EXIST);
				
				// Create object to return.
				$return = new stdClass();
				$return->id = (int) $message->id;
				$return->useridfrom = (int) $message->useridfrom;
				$return->nomefrom = $nomefrom->firstname." ".$nomefrom->lastname;
				$return->useridto = (int) $message->useridto;
				$return->nometo = $nometo->firstname." ".$nometo->firstname;
				$return->subject = $message->subject;
				//$return->smallmessage = htmlentities(urlencode(strip_tags($message->smallmessage)));
				$return->smallmessage = urlencode(strip_tags($message->smallmessage));
				$return->timecreated = $message->timecreated;
				$arrmessages[$return->id] = (array) $return;
			}
		}
        ...
        $log = $DB->insert_record('log', array('userid' => $dados['userid'],
													...
													'ip' => $_SERVER['REMOTE_ADDR'],
													'module' => "Mobile",
													'action' => "Modulo - Mensagens",
													'url' => "",
													'info' => "Carregando mensagens"));
		
        return $arrmessages;
    }

    /**
     * Describes the get_messages return value.
     *
        ...
        return new external_multiple_structure(
            new external_single_structure(
                array(
                    ...
                    'nomefrom' => new external_value(PARAM_TEXT, 'name from'),
                    'useridto' => new external_value(PARAM_INT, 'user id to'),
                    'nometo' => new external_value(PARAM_TEXT, 'name to'),
                    'subject' => new external_value(PARAM_TEXT, 'Message subject'),
                    'smallmessage' => new external_value(PARAM_TEXT, 'Full message'),
            ...
            array(
                'dados' => new external_single_structure(
                    array(
                        ...
                        'userid' => new external_value(PARAM_INT, 'id of user'),
						'smallmessage' => new external_value(PARAM_TEXT, 'Message name')
                    )
                )
            )
        );
     ...
     * @return string return
     */
    public static function reply_message($dados) {
        global $CFG, $DB, $USER;
        ...
        //Parameter validation
        //REQUIRED
        $params = self::validate_parameters(self::reply_message_parameters(),
                array('dados' => $dados));
		...
		// get the messages parent data
		$messages = $DB->get_record('message', array('id' => $dados['id']), '*', MUST_EXIST);

		// Reply the message.
        $reply = $DB->insert_record('message', array('useridfrom' => $dados['userid'],
													'useridto' => $messages->useridfrom,
													'subject' => "New message",
													'fullmessage' => $dados['smallmessage'],
													'smallmessage' => $dados['smallmessage'],
													'timecreated' => $timestamp));
													
		// delete old message.
        $delete = $DB->delete_records('message', array('id' => $dados['id']));
    ...
    public static function get_grades($grades) {
		...
		$response = strip_tags_deep($response);
		
		// log.
        $log = $DB->insert_record('log', array('userid' => $USER->id,
													'time' => mktime(date('H'),date('i'),date('s'),date('m'),date('d'),date('Y')),
													'ip' => $_SERVER['REMOTE_ADDR'],
													'module' => "Mobile",
													'action' => "Modulo - Notas",
													'url' => "",
													'info' => "Carregando notas de cursos."));
                ...
                'items'  => new external_multiple_structure(
                    new external_single_structure(
                        array(
                            ...
                            'grademin' => new external_value(PARAM_TEXT, 'Minimum grade'),
                            'grademax' => new external_value(PARAM_TEXT, 'Maximum grade'),
                            'gradepass' => new external_value(PARAM_TEXT, 'The passing grade threshold'),
                            ...
                            'grades' => new external_multiple_structure(
                                new external_single_structure(
                                    array(
                                        ...
                                        'feedback' => new external_value(PARAM_TEXT, 'Feedback from the grader'),
    ...
    public static function get_forums_by_courses($courseids = array()) {
        ...
        $log = $DB->insert_record('log', array('userid' => $USER->id,
													...
													'ip' => $_SERVER['REMOTE_ADDR'],
													'module' => "Mobile",
													'action' => "Modulo - Forum de Cursos",
													'url' => "",
													'info' => "Carregando dados de forum de cursos."));
